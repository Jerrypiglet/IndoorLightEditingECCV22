...
# switches
parser.add_argument('--ifVisLamp', type=str2bool, nargs='?', const=True, default=True, help='whether to use visible lamp in rendering')
parser.add_argument('--ifVisWin', type=str2bool, nargs='?', const=True, default=True, help='whether to use visible window in rendering')
...
for dataId in range(max(opt.rs, 0), min(opt.re, len(dirList ) ) ):
    ...
    if opt.objName is None:
        ...
        depthBatch = torch.from_numpy(np.load(depthName ) ).cuda().float()
        if len(depthBatch.shape ) == 2:
            depthBatch = depthBatch.unsqueeze(0).unsqueeze(0)
        depthDSName = osp.join(inputDir, 'depthDS.npy')
        depthDS = torch.from_numpy(np.load(depthDSName ) ).cuda().float()
    else:
        depthName = osp.join(inputDir, 'depth.npy')
    ...
    for n in range(0, visLampNum ):
        ...
        with open(lampName, 'rb') as fIn:
            ...
            print(magenta('Lamp info:'), lampInfo)
    ...
    for n in range(0, visWinNum ):
        ...
        if not opt.ifVisWin:
            winSrc = winSrc * 0.
            winSrcSky = winSrcSky * 0.
            winSrcGrd = winSrcGrd * 0.
            print(red('Turned off vis windows'))
    ...
    if not opt.ifInvisLamp:
        invLampSrcPred = invLampSrcPred * 0.
    ...
    if not opt.ifInvisWin:
        invWinSrcPred = invWinSrcPred * 0.
        invWinSrcSkyPred = invWinSrcSkyPred * 0.
        invWinSrcGrdPred = invWinSrcGrdPred * 0.
        print(red('Turned off invisible window'))

    if opt.objName is None:
        
        print(depthDS.shape, depthDS.dtype, torch.amax(depthDS), torch.amin(depthDS)) # torch.Size([1, 1, 120, 160]), torch.float32, tensor(5.2797, device='cuda:0') tensor(1.4213, device='cuda:0')
        print(normalDS.shape, normalDS.dtype, torch.amax(normalDS), torch.amin(normalDS)) # torch.Size([1, 3, 120, 160]), torch.float32, tensor(0.9994, device='cuda:0') tensor(-0.9999, device='cuda:0')
        print(visWinCenterPreds) # 0
        print(len(visWinXAxisPreds)) # 0
        print(len(visWinYAxisPreds)) # 0
        print(len(winMaskSmallBatch)) # 0
        print(len(visLampCenterPreds)) # 0
        print(len(lampMaskSmallBatch)) # 0
        print(invWinCenterPred.shape, invWinCenterPred.dtype, torch.amax(invWinCenterPred), torch.amin(invWinCenterPred)) # torch.Size([1, 3]) torch.float32 tensor(0.0021, device='cuda:0') tensor(-0.6355, device='cuda:0')
        print(invWinXAxisPred.shape, invWinXAxisPred.dtype, torch.amax(invWinXAxisPred), torch.amin(invWinXAxisPred)) # torch.Size([1, 3]) torch.float32 tensor(2.0929, device='cuda:0') tensor(0.2807, device='cuda:0')
        print(invWinYAxisPred.shape, invWinYAxisPred.dtype, torch.amax(invWinYAxisPred), torch.amin(invWinYAxisPred)) # torch.Size([1, 3]) torch.float32 tensor(0.3980, device='cuda:0') tensor(-0.0509, device='cuda:0')
        print(invLampAxesPred.shape, invLampAxesPred.dtype, torch.amax(invLampAxesPred), torch.amin(invLampAxesPred)) # torch.Size([1, 3, 3]) torch.float32 tensor(0.5073, device='cuda:0') tensor(-0.5973, device='cuda:0')
        print(invLampCenterPred.shape, invLampCenterPred.dtype, torch.amax(invLampCenterPred), torch.amin(invLampCenterPred)) # torch.Size([1, 3]) torch.float32 tensor(0.8977, device='cuda:0') tensor(-0.8978, device='cuda:0')
    ...
    # print(albedoDS.dtype)
    # print(normalDS.dtype)
    # print(depthDS.dtype)
    # print(shadingDirectPredInput.dtype)
    # print(onMaskSmallBatch.dtype)
    ...
    print(white_blue('Rendering saved to: %s' % renderedHdrName))
